...
contract SpotBattle is DataLayout, Proxiable{
    ...
    function setClaimHistory(address user, uint256 index, uint256 round) public {
        userClaimHistory[user][index][markets[0]][round] = false;
    }
    ...
    function claim(address user, uint256 userIndex) public {
        ...
        require(!userClaimHistory[user][userIndex][token][round], "Already claimed");
        ...
        uint256 winnings;
        if (longWon) {
            if(marketData[token].shortHistory[round-1] > 0 && marketData[token].longHistory[round-1] > 0) {
                winnings = BNBUsed.mul(marketData[token].shortHistory[round-1]).div(marketData[token].longHistory[round-1]);
            }
        } else {
            if(marketData[token].longHistory[round-1] > 0 && marketData[token].shortHistory[round-1] > 0) {
                winnings = BNBUsed.mul(marketData[token].longHistory[round-1]).div(marketData[token].shortHistory[round-1]);   
            }
        }
        winnings = winnings.add(BNBUsed);
        ...
        require(winnings > 0, "0 BNB value");
        (bool sent, bytes memory data) = payable(user).call{value: winnings}("");
 
        //send user Versus as reward, if not free prediction, how much though?

        userClaimHistory[user][userIndex][token][round] = true;
        VersusToken(versusToken).updateUserWins(user);
...
interface VersusToken {
    ...
    function updateUserWins(address _user) external;
}